...
const AdminMediaList: React.VFC = () => {
  ...
  const { data } = useSWR(`db/media/detail/${permalink}`, async () => {
    const { docs } = await getDocs(query(collections.media, where('permalink', '==', permalink), limit(1)));
  ...
  const onSubmit = async (values: MediaFormValues) => {
    ...
    await updateDoc(doc(collections.media, data.id), { ...values, updatedAt: serverTimestamp() });
