...
const fetchCMSContents = async (): Promise<void> => {
  ...
  const [settings, works, music, performances] = await Promise.all([
    ...
    client.get<MicroCMSApiArrayResponse<MicroCMSApiWorksSchema>>({
      endpoint: 'works',
    }),
    client.get<MicroCMSApiArrayResponse<MicroCMSApiMusicSchema>>({
      endpoint: 'music',
    }),
    client.get<MicroCMSApiArrayResponse<MicroCMSApiPerformanceSchema>>({
